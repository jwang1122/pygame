...
class HitChimp(AppSuper):
    ...
    def paint(self): # leave this function for subclass to implement
        ...
        # keep chimp inside the screen
        self.chimp.rect.left += self.xSpeed
        self.chimp.rect.top += self.ySpeed
        if self.chimp.rect.left < 0 or self.chimp.rect.right > HitChimp.width:
            self.xSpeed = -self.xSpeed
        if self.chimp.rect.top <0 or self.chimp.rect.bottom > HitChimp.height:
            self.ySpeed = -self.ySpeed
